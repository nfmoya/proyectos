...
-----------------------------------------------------------------------------
-- DDL for Stored Procedure 'CFACSERV.dbo.sp_CFS_anularConciliacion;1'
-----------------------------------------------------------------------------

print '<<<<< CREATING Stored Procedure - "CFACSERV.dbo.sp_CFS_anularConciliacion;1" >>>>>'
go 

use CFACSERV
go

IF EXISTS (SELECT 1 FROM sysobjects o, sysusers u WHERE o.uid=u.uid AND o.name = 'sp_CFS_anularConciliacion' AND u.name = 'dbo' AND o.type = 'P')
BEGIN
	setuser 'dbo'
	drop procedure sp_CFS_anularConciliacion
...
IF (@@error != 0)
BEGIN
	PRINT "Error CREATING Stored Procedure 'CFACSERV.dbo.sp_CFS_anularConciliacion;1'"
	SELECT syb_quit()
END
go
...
CREATE PROCEDURE sp_CFS_anularConciliacion(
@cdConciliacion  int,
...
begin  
    ...
    if ((select ST_CONCILIACION from TCF013_CONCILIA WHERE CD_CONCILIACION = @cdConciliacion) = 'ANU') begin
        set @po_c_error = 2
    ...
    begin tran anulacion
        ...
        -- Se cambia estado a las diferencias asociadas de la conciliacion.
        if ((select COUNT(CD_CONCILIACION) from TCF014_DIFCONCILIA WHERE CD_CONCILIACION = @cdConciliacion) > 0) begin
            update TCF014_DIFCONCILIA set ST_DIFERENCIA = 'ANU' WHERE CD_CONCILIACION = @cdConciliacion
        ...
        update TCF013_CONCILIA set ST_CONCILIACION = 'ANU', NB_OBSERVACIONES = @observaciones  WHERE CD_CONCILIACION = @cdConciliacion
        ...
        update TCF012_SERVFACDET set CD_CONCILIACION = null, FH_MODIFICACION = getdate(), NB_USUARIOMODIF = @userAnulacion WHERE CD_CONCILIACION = @cdConciliacion
        ...
        select  @logCdConciliacion = CD_CONCILIACION, 
            ...
            from TCF013_CONCILIA WHERE CD_CONCILIACION = @cdConciliacion
...
sp_procxmode 'sp_CFS_anularConciliacion', unchained
go 
