...
-----------------------------------------------------------------------------
-- DDL for Stored Procedure 'CFACSERV.dbo.sp_CFS_Consulta_ServPrest;1'
-----------------------------------------------------------------------------

print '<<<<< CREATING Stored Procedure - "CFACSERV.dbo.sp_CFS_Consulta_ServPrest;1" >>>>>'
go 

use CFACSERV
go

IF EXISTS (SELECT 1 FROM sysobjects o, sysusers u WHERE o.uid=u.uid AND o.name = 'sp_CFS_Consulta_ServPrest' AND u.name = 'dbo' AND o.type = 'P')
BEGIN
	setuser 'dbo'
	drop procedure sp_CFS_Consulta_ServPrest
...
IF (@@error != 0)
BEGIN
	PRINT "Error CREATING Stored Procedure 'CFACSERV.dbo.sp_CFS_Consulta_ServPrest;1'"
	SELECT syb_quit()
END
go
...
CREATE PROCEDURE dbo.sp_CFS_Consulta_ServPrest( 
...
if(@pi_DESDE=null and @pi_CD_SECTOR=null and @pi_CD_PRODUCTO=null) 
begin 
...
begin
	...
	begin
        ...
        begin
		    select @pi_DESDE = convert(varchar,min(FH_REMITO),103)   from TCF009_SERVPRESCAB b left join TCF010_SERVPRESDET a  on (b.CD_LOTESERV = a.CD_LOTESERV )
				where CD_PROVEEDOR  = @pi_CD_PROVEEDOR and CD_SECTOR = @pi_CD_SECTOR and CD_PRODUCTO = @pi_CD_PRODUCTO
        end
	end
end

if(@pi_HASTA =null and @pi_CD_SECTOR =null and @pi_CD_PRODUCTO =null) 
begin 
...
BEGIN
    ...
    if(@pi_CD_PROVEEDOR  = null or @pi_CD_PROVEEDOR  = '0')
    begin
        print 'setea proveedor'
...
if(@pi_NM_LOTE <> null AND @pi_NM_LOTE <> 0)
    ...
    begin 
		...
		if(@pi_CD_SECTOR = null and @pi_CD_PRODUCTO =null) 
		begin 
		  insert into  #tempServPrest 
		  SELECT DISTINCT 
			  t009.CD_PROVEEDOR as prov , t010.CD_SECTOR as sect, P.CD_GRUPOPRODUCTO as grup,
			  t010.CD_PRODUCTO  as prod , dbo.periodoDesdeFecha(t010.FH_REMITO) as periodo
			   FROM  TCF009_SERVPRESCAB t009 
			   LEFT join TCF010_SERVPRESDET t010 on t009.CD_LOTESERV = t010.CD_LOTESERV
			   LEFT JOIN TCF005_PRODUCTO P ON  t010.CD_PRODUCTO = P.CD_PRODUCTO and  t009.CD_PROVEEDOR = P.CD_PROVEEDOR
			...
			begin
				...
				SELECT DISTINCT 
				  ...
				  t010.CD_PRODUCTO  as prod , dbo.periodoDesdeFecha(t010.FH_REMITO) as periodo
				   ...
				   where t009.CD_PROVEEDOR = @pi_CD_PROVEEDOR
						and t010.CD_SECTOR = @pi_CD_SECTOR
						and t010.CD_PRODUCTO = @pi_CD_PRODUCTO
				   and t010.FH_REMITO >=  (convert(date, @pi_DESDE ,103))  --'03 Jan 2011'
				   and t010.FH_REMITO <=  (convert(date, @pi_HASTA ,103)) --'03 Jan 2015'
					AND t010.ST_LOTEDET = @pi_ST_LOTEDET 
			end
...
set @query = 'SELECT (t010.CD_SECTOR + '' - '' + t007.NB_SECTOR) as CD_SECTOR,  '
	...
	+'(t010.CD_PRODUCTO + '' - '' + P.NB_PRODUCTO) as CD_PRODUCTO '       
while (@@sqlstatus != 2)
begin    
    print 'vuelta'
    set @query = @query + 
...
set @query = @query + ', sum(t010.CT_SERVPREST )as SERVPREST   '
  ...
  +' , count(case when t010.CD_CONCILIACION = null then t010.CD_REMITO  else (case when ST_CONCILIACION = ''GRA'' then CD_REMITO else null end ) end ) as TOTALREMITOSACON  '
...
if(@pi_CD_PRODUCTO <> ''  and @pi_CD_PRODUCTO <> null    and @pi_CD_PRODUCTO <> '0'  )
  begin
	  set @query = @query +   ' and t010.CD_PRODUCTO = ''' ||  @pi_CD_PRODUCTO  ||  '''' 
  end
...
set @query = @query + ' GROUP BY t010.CD_SECTOR, P.CD_GRUPOPRODUCTO, t010.CD_PRODUCTO, t007.NB_SECTOR, t001.NB_CODTABLA, P.NB_PRODUCTO '+
'ORDER BY t010.CD_SECTOR, P.CD_GRUPOPRODUCTO, t010.CD_PRODUCTO'
...
sp_procxmode 'sp_CFS_Consulta_ServPrest', unchained
go 
