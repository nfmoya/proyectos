...
-----------------------------------------------------------------------------
-- DDL for Stored Procedure 'CFACSERV.dbo.sp_CFS_busqueda_diferencias;1'
-----------------------------------------------------------------------------

print '<<<<< CREATING Stored Procedure - "CFACSERV.dbo.sp_CFS_busqueda_diferencias;1" >>>>>'
go 

use CFACSERV
go

IF EXISTS (SELECT 1 FROM sysobjects o, sysusers u WHERE o.uid=u.uid AND o.name = 'sp_CFS_busqueda_diferencias' AND u.name = 'dbo' AND o.type = 'P')
BEGIN
	setuser 'dbo'
	drop procedure sp_CFS_busqueda_diferencias
...
IF (@@error != 0)
BEGIN
	PRINT "Error CREATING Stored Procedure 'CFACSERV.dbo.sp_CFS_busqueda_diferencias;1'"
	SELECT syb_quit()
END
go
...
CREATE PROCEDURE dbo.sp_CFS_busqueda_diferencias ( 
    ...
    */
    ...
    INNER JOIN TCF013_CONCILIA t13     ON t14.CD_CONCILIACION = t13.CD_CONCILIACION
    INNER JOIN TCF005_PRODUCTO t05     ON t13.CD_PRODUCTO = t05.CD_PRODUCTO   AND t13.CD_PROVEEDOR = t05.CD_PROVEEDOR 
    INNER JOIN TCF003_PERIODOFACT t03  ON t13.CD_PROVEEDOR = t03.CD_PROVEEDOR AND t13.CD_PERIODOFACT = t03.CD_PERIODOFACT
    WHERE t13.CD_PROVEEDOR    =  (case when @pi_cd_proveedor is null or @pi_cd_proveedor='0' then t13.CD_PROVEEDOR else @pi_cd_proveedor end)         
      ...
      and ((t13.ST_CONCILIACION IN ('PEN', 'GRA') and @pi_cd_conciliacion = -1) OR (t13.ST_CONCILIACION IN (t13.ST_CONCILIACION) and @pi_cd_conciliacion != -1))
  end
    ...
    begin
        set @po_c_error = 1
        set @po_d_error = ' Error en Busqueda de Diferencias de Conciliaciones '
...
sp_procxmode 'sp_CFS_busqueda_diferencias', unchained
go 
