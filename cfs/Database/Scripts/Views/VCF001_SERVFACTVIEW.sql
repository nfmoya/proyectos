...
-----------------------------------------------------------------------------
-- DDL for View 'CFACSERV.dbo.VCF001_SERVFACTVIEW'
-----------------------------------------------------------------------------

print '<<<<< CREATING View - "CFACSERV.dbo.VCF001_SERVFACTVIEW" >>>>>'
go 

use CFACSERV
go

IF EXISTS (SELECT 1 FROM sysobjects o, sysusers u WHERE o.uid=u.uid AND o.name = 'VCF001_SERVFACTVIEW' AND u.name = 'dbo' AND o.type = 'V')
BEGIN
	setuser 'dbo'
	drop view VCF001_SERVFACTVIEW
...
IF (@@error != 0)
BEGIN
	PRINT "Error CREATING View 'CFACSERV.dbo.VCF001_SERVFACTVIEW'"
	SELECT syb_quit()
END
go
...
SELECT t012.CD_SECTORCONCIL as CD_SECTORCONCIL
      ...
      ,(t012.IM_PRECIOTOTAL * (select t004.NU_VALBRUTOUNIVAL from TCF004_VALORUNIVAL t004 where t004.CD_UNIVAL     = P.CD_UNIVAL 
                                        and t004.CD_PROVEEDOR   = t011.CD_PROVEEDOR
